package com.barclays.microservices.monopoly.player;
...
public class Player {
	...
	private List<Cell> properties = new ArrayList<Cell>();
	private List<Cell> railroads = new ArrayList<Cell>();
	private List<Cell> utilities = new ArrayList<Cell>();

	public Player() {

		GameBoard gb = new GameMaster().getGameBoard();
		inJail = false;
		if (gb != null) {
	...
	public void buyProperty(Cell property, int amount) {
		...
		if (property.getColorGroup().equals(PropertyCell.COLOR_GROUP)) {
		...
		if (property.getColorGroup().equals(RailRoadCell.COLOR_GROUP)) {
			railroads.add(property);
			colorGroups.put(RailRoadCell.COLOR_GROUP,
					new Integer(getPropertyNumberForColor(RailRoadCell.COLOR_GROUP) + 1));
		}

		if (property.getColorGroup().equals(UtilityCell.COLOR_GROUP)) {
			utilities.add(property);
			colorGroups.put(UtilityCell.COLOR_GROUP,
	...
	public Cell[] getAllProperties() {
		...
		allCells.addAll(properties);
		allCells.addAll(utilities);
		allCells.addAll(railroads);
		return (Cell[]) allCells.toArray(new Cell[allCells.size()]);
	}
	...
	public void purchaseHouse(String selectedMonopoly, int houses) {
		...
		GameBoard gb = gameMaster.getGameBoard();
